...
public class TelaReceitas extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        lblReceitas.setFont(new java.awt.Font("SimSun", 1, 18)); // NOI18N
        lblReceitas.setForeground(new java.awt.Color(255, 255, 255));
        lblReceitas.setText("Receitas");

        tabelaReceitas.setForeground(new java.awt.Color(0, 0, 0));
        tabelaReceitas.setModel(montarTabela());
        tabelaReceitas.setSelectionBackground(new java.awt.Color(150, 193, 107));
        ...
        btnExcluir.setText("Excluir Receita");
    ...
    private void btnExcluirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnExcluirActionPerformed
        try {
            int id = getPosicao();
            int resposta = JOptionPane.showConfirmDialog(null, "Tem certeza de que deseja excluir essa receita?");
            if (resposta == 0) {
                ReceitaJPA.excluirReceita(id);                
            }
        } catch (ArrayIndexOutOfBoundsException e) {
            JOptionPane.showMessageDialog(null, "Selecione a receita que deseja excluir!", "Erro!", 2, null);
        }
    ...
    private void btnPesquisarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPesquisarActionPerformed
        tabelaReceitas.setModel(montarTabelaFiltrada(txtPesquisar.getText()));
    ...
    private int getPosicao() {
        int posicao = tabelaReceitas.getSelectedRow();
        return Integer.parseInt((String) tabelaReceitas.getValueAt(posicao, 0));
    }
    
    private DefaultTableModel montarTabelaFiltrada(String filtro) {
        String[] colunas = {"ID", "Pet", "Receita"};
        
        DefaultTableModel tabela = new DefaultTableModel(colunas, 0);
        
        List<Receita> lista = ReceitaJPA.listarReceitasNome(filtro);
        
        for (int i = 0; i < lista.size(); i++) {
            
            Receita r = lista.get(i);
            
            String[] linha = {
                Integer.toString(r.getId()),
                r.getConsulta().getPet().getNome(),
                r.getReceita()
    ...
    private DefaultTableModel montarTabela() {
        ...
        List<Receita> lista = ReceitaJPA.listarReceitas();
